...
## プロジェクト概要
　クライアント企業様（システム開発会社）において、現在の顧客管理システムのクラウド化を含む刷新作業を進める中で、私たちは内部の開発・運用・改善活動にAIを取り入れました。  
当初は単なる自動化支援として始めたAI活用でしたが、対話を通じた設計・レビュー・テストの高速化を重ねるうちに、チームの働き方そのものが徐々にアジャイル、特にSCRUMに近づいていきました。  
この資料では、その変化の背景と、現場で実際に起きた「自然発生的なアジャイル化」の過程を紹介します。

この取り組みの本質は、AIを導入したことそのものではなく、ROI最大化を目指して価値提供の速度と品質を高めるために、AIを最大の相棒として使い続けたことにあります。再春館システム様での実践では、要件の曖昧さを対話で解きほぐし、設計・実装・テストを短いサイクルで回すことで、単なる自動化ではなく、Value（価値提供）を大きくしながらCost（自働化・効率化）を下げる、いわばAIを前提にしたアジャイル／SCRUM的な取り組みが自然に形になりました。
