...
<img src="/images/vive/福岡支社質疑応答.jpg" alt="福岡支社質疑応答" />
...
2015年頃、AI（CNNやRNN）は「猫の画像」を見て「猫」と答えることはできましたが、それが「なぜ猫なのか」を説明することはできませんでした。
今でも当時の認識通り、AIの仕組みは<strong>「統計的な相関関係（パターンマッチング）」</strong>であり、それは現在も変わっていません。

<img src="/images/vive/AIが意味を理解する仕組み.jpg" alt="AIが意味を理解する仕組み" />

また、AIは言葉の意味を理解しているわけではありません。すべての言葉を<strong>「Word Embedding（単語埋め込み）」</strong>という技術によって<strong>「ベクトル（数百〜数千次元の数字の列）」</strong>に変換し、ベクトル同士の<strong>「向き（コサイン類似度）」</strong>が近いかどうかで、「意味の近さ」を判定しています。
さらに革新的なのは、この技術が「言葉」と「画像」といった、本来は次元の異なるデータ同士をも繋げたことです。 例えば、「猫」という文字と「猫の写真」を同じベクトル空間の座標に変換することで、AIはそれらが「同じ意味（近い座標）」であると計算で理解できようになります。

この、あらゆるデータを共通の座標で扱う<strong>「ベクトル計算」</strong>こそが、マルチモーダルなAIの実体なのです。
...
<img src="/images/vive/2017年の革命と「NVIDIA一強」の理由.jpg" alt="C2017年の革命と「NVIDIA一強」の理由" />

### 核心技術：Attentionと文脈学習

このTransformerの中心にあるのが、**Attention（注意機構）** です。
これは、文章の中の「どの単語」と「どの単語」が強く関連しているかを、あたかも人間が注目するように計算する仕組みです。
この機構のおかげで、AIは再学習をすることなく、プロンプトに入力された情報をその場で理解してタスクに適応する **文脈学習（In-Context Learning）** という能力を手に入れました。私たちがAIに「この規約を守って」と指示するだけで従うのは、この能力のおかげなのです。



### 盤石な基盤：CUDA（クーダ）

ここでハードウェアの運命が決まりました。この巨大な並列計算を処理できるのは、CPU（指揮官）ではなく、単純な計算を大量にこなすGPU（数千人の兵隊）だったからです。
...
<img src="/images/vive/スケーリング則：計算量と性能の関係.jpg" alt="スケーリング則：計算量と性能の関係" />
...
この「巨大な汎用脳」を、私たちの現場（Vive-with-Gemini）で「相棒」にするために使われているのが、**RAG**と**LoRA**です。
 ...
 <img src="/images/vive/現場での活用技術：RAGとLoRA.jpg" alt="現場での活用技術：RAGとLoRA" />
  ...
  * **LoRA（ローラ：適応学習＜専門教育を施す＞）：**
    巨大なモデル本体（脳）を再学習させるのは最低でも数億円以上かかります。そこで、脳の外側に「薄いフィルター（アダプター）」だけをくっつけて、そこだけを学習させる技術です。これにより、低コストで<strong>「チーム専用の性格」</strong>を持たせることができます。
...
私たちがコードを書かせたりレビューをさせたりするたびに、裏側ではGPUが推論の計算を回しています。また推論には2つの潮流があります。
...
<img src="/images/vive/学習と推論.jpg" alt="学習と推論" />

## 6. 最新動向：NVIDIA一強の終焉？ TPUと「Gemini 3」の衝撃（2025年12月追記）
...
次世代モデル（Gemini 3.0など）の学習において、NVIDIAのGPUではなく、自社開発のAI専用チップである<strong>TPU（Tensor Processing Unit）</strong>を全面的に使用して構築したとされています。
...
これまで「AIへの投資＝NVIDIAへの課金（70％以上が粗利）」と等しい状態でしたが、Googleが自社チップだけで世界最高峰のモデル（Gemini 3.0）を作り上げた事実は、<strong>「NVIDIAへの依存なしでもトップティアのAIは作れる」</strong>という強力な証明となりました。
...
<img src="/images/vive/最新動向：NVIDIA一強の終焉？TPUと自社シリコンの衝撃.jpg" alt="最新動向：NVIDIA一強の終焉？TPUと自社シリコンの衝撃" />
...
<img src="/images/vive/Vive-with-Geminiが相棒と呼ぶ理由.jpg" alt="Vive-with-Geminiが相棒と呼ぶ理由" />
...
<img src="/images/vive/次回予告：賢さから行動へ.jpg" alt="次回予告：賢さから行動へ" />
