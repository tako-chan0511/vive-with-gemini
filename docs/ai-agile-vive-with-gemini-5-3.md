...
- GuideLLMを共通手順で実行するベンチマークテンプレート
...
7. 人が元データと生成結果を確認し、モデル選定とGPU資源設計を最終判断する
...
### MaaSベンチマーク評価・分析の自動化フロー

これは業務フローであり、物理的なネットワーク構成を示す図ではありません。
点線は、人とルールを介した所定の受け渡しを表します。

![実行、保存、分析支援、人による判断をつなぐMaaSベンチマーク評価・分析フロー](./public/images/vive/maas-benchmark-flow.svg)
...
AI協働、MaaS実行基盤、モデル評価自動化、適応型開発を分断せず、
一つのループとして回します。
...
これらは測定済みの実績ではなく、定義と測定条件を整える候補です。
...
GuideLLMとExcelでモデル評価を同じ手順へそろえる。
