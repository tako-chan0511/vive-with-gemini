...
- Microsoft 365 Copilotへ渡す定型の分析手順と依頼文
...
- GuideLLMの結果とOpenShiftメトリクスのグラフを突き合わせ、レポート作成を自動化
...
保存したデータは所定のExcelテンプレートへ取り込み、
Microsoft 365 Copilotへ定型の依頼文で分析を依頼します。

1. GuideLLMでベンチマークを実行する
2. 評価条件と結果を共通形式で保存する
3. 保存データをExcelテンプレートへ取り込む
4. Microsoft 365 Copilotが集計・分析を支援する
5. モデル別・条件別の比較表を作成する
6. スループットやレイテンシーなどをグラフ化する
7. 人が性能データを確認し、回答品質などの別評価も踏まえて、モデル採用とGPU資源設計を最終判断する
...
同じ時間軸で推論処理と資源利用の関係を見ますが、
時間同期したグラフだけで因果関係を断定しません。

現時点では、PromQLの選択、表示期間の同期、
GuideLLMの結果との突き合わせを人手で行っています。
この一連の作業は、今後の自動化・改善候補です。
...
- 保存した評価データを所定の列と形式で取り込む
- 分析手順とMicrosoft 365 Copilotへ渡す定型の依頼文を含める
- モデル別・条件別の比較表を作る
