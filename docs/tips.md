...
最も手軽で強力な方法が、VSCodeに標準搭載されているGitの差分表示機能です。
...
これにより、コマンドを一切使わずに、意図しない変更がないかを視覚的に、かつ正確に確認できます。

#### **検証テクニック②：`diff`コマンドで確認する**
ターミナルでの操作に慣れている場合は、`diff`コマンドも有効です。

1.  AIが生成したコードを一時ファイル（例: `App.gemini.vue`）として保存します。
2.  ターミナルで `diff -u src/App.vue App.gemini.vue` を実行します。
3.  追加行（`+`）、削除行（`-`）で差分が表示されます。
