...
# 私たちのTips集 (vive-with-gemini実践編)

このページでは、私と相棒のGeminiが「vive-with-gemini」を実践する中で見出した、効果的なコツや問題への対処法を、**実践の流れに沿った4つのステップ**で共有します。

## ステップ1：心構えを整える（マインドセット編）

vive-with-geminiを始める前に、まず私たち自身の役割を再定義することが、AIとの協業を成功させる最大の鍵となります。
...
vive-with-geminiの最中に、AIが突然、文脈を無視した頓珍漢な回答を繰り返すことがあります。そんな時、最も効果的で、ROIの高い（＝時間的コストが最も低い）解決策は、**一度深呼吸し、冷静なナビゲーターに徹する**ことです。
...
### Tip7: 「プロンプトの魔術師」ではなく「対話の上手い開発者」を目指す

「魔法のようなプロンプト」を一発で書くスキルよりも、優れたエンジニアとしての「対話」のスキルの方が、AIとの協業ではるかに重要です。完璧な指示を目指すのではなく、以下の3つのスキルを駆使して、対話的にAIを正解に導きます。

1.  **テストで「正解」を定義する能力:** 何が「正しい動作」なのかを、まずテストコードでAIに伝えます。これが客観的な仕様書となり、AIの提案を判定する基準になります。
2.  **的確な「コンテキスト」を与える能力:** テストの失敗、既存コードの制約、達成したい目的といった、AIが判断するために必要な情報を過不足なく与えます。AIの間違いは、より良いコンテキストを与えるチャンスです。
3.  **コードの「良し悪し」を判断する能力:** AIが生成したコードが、要件を満たしているかだけでなく、保守性や拡張性の観点から見て適切かどうかをレビューします。

私たちの役割は、特殊な呪文を唱えることではありません。むしろ、「非常に高速だが経験の浅いジュニアプログラマー（AI）」を的確に導ける、優れたシニアエンジニアの振る舞いに近いと言えるでしょう。

### Tip8: 画面のエラーやUIの相談は、画像を添付する

テキストで状況を説明するよりも、**スクリーンショットなどの画像を添付した方が、はるかに高精度な回答**が返ってきます。画像は、私たちが言葉にしづらいUIのレイアウトやコンソールの状態といった、膨大な周辺情報をAIに伝えてくれます。

### Tip9: AIの提案が複雑な時は「完成版」を丸ごと依頼する
