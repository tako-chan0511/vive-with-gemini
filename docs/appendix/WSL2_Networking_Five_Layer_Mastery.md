...
### 詳細解説
- WSL2は「LinuxがWindowsの中で動いている」ように見えますが、ネットワーク的には **小さな仮想マシン（VM）** として動作します。
- そのため、通信問題は「どこで詰まっているか」を層（レイヤー）で切り分けるのが最短です。
...
- いきなり設定変更しないで、まずは **現状の観測（show）** を揃えるのがコツです。  
  「何が正常で、何が異常か」を言語化できると、調査が一気に速くなります。
...
### 詳細解説
- 典型症状と対応レイヤー：
  - `ping 8.8.8.8` がNG → Layer2（経路）〜Layer3（フィルタ）
