## １ 表紙：The Digital Handshake（デジタルの握手）
...
## ４ HTTPメソッド：動作はURLではなくメソッドで表現する
...
## １０ 情報漏洩を防ぐ：レスポンスは“許可リスト”で組み立てる
