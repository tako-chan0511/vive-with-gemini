...
PowerPoint／LibreOfficeをすべて終了してから、`presentation` 内に
`~$` で始まる同名ファイルや `.~lock.` で始まるファイルが残っていないか
確認してください。これらは編集中だけ作られるロックファイルです。本体と一緒に
Desktopからコピーすると、PowerPointが「別のユーザーが編集中」と判断して
読み取り専用で開くことがあります。

- コピーするのは実体の `.pptx` だけにする
- 同じ `.pptx` をPowerPointとLibreOfficeで同時に開かない
- スクリプトは `sudo` を付けず、通常ユーザーで実行する
- PowerPointが起動中のロックファイルは削除しない
